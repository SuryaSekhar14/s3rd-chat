...
export const useDatabase = () => {
  ...
  const { conversations, refresh: refreshConversations, mutate } = useConversationsSWR();
  ...
  const createConversationInDatabase = useCallback(async (conversationId: string, title?: string) => {
  ...
  const updateConversationInDatabase = useCallback(async (conversationId: string, updates: { title?: string }) => {
    if (!user?.id || !isLoaded) return false;
  ...
  const deleteConversationFromDatabase = useCallback(async (conversationId: string) => {
    if (!user?.id || !isLoaded) return false;
    ...
    try {
      ...
      mutate((current: any) => {
        ...
        const filteredConversations = current.conversations.filter((conv: any) => conv.id !== conversationId);
  ...
  const saveMessagesToDatabase = useCallback(async (conversationId: string, messages: ChatMessageJSON[]) => {
    if (!user?.id || !isLoaded) return false;
  ...
  const getSpecificConversation = useCallback(async (conversationId: string) => {
    if (!user?.id || !isLoaded) return null;
